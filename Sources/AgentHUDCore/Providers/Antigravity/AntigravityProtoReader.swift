...
struct AntigravityProtoReader {
    ...
    private struct Timestamp {
        ...
        func milliseconds() throws -> Int64? {
            ...
            else { throw AntigravityProtoFailure.invalid }
    ...
    struct Field {
        ...
        func message() throws -> ArraySlice<UInt8> {
            guard self.wire == 2, let data else { throw AntigravityProtoFailure.invalid }
            return data
        }

        func integer() throws -> UInt64 {
            guard self.wire == 0, let value else { throw AntigravityProtoFailure.invalid }
            return value
        }

        func counter() throws -> Int {
            guard let value = try Int(exactly: self.integer()) else {
                throw AntigravityProtoFailure.invalid
            }
            return value
        }
        ...
        visit: (Field) throws -> Void) throws
    {
        ...
        guard !reader.isMalformed else { throw AntigravityProtoFailure.invalid }
    }
        ...
        checkCancellation: () throws -> Void) throws
    {
        try self.fields(bytes, checkCancellation: checkCancellation) { stamp in
            ...
            case 2:
                ...
                guard nanos <= 999_999_999 else { throw AntigravityProtoFailure.invalid }
                time.nanos = nanos
            default: break
            }
        }
