...
- Clicking an event opens the statistics window on what it is about: a completed turn on its session's page in Sessions, a quota event on its window's tile in Tokens, pointed out for a moment, and anything else on Tokens. A session row of the panel opens that session's page and its heading the Sessions list; the chart button and the menu bar's rows open Tokens, with Sessions back at its list.
