...
- The Tokens page prices the charted range and each period from 15-minute buckets, each model on its client's platform and DeepSeek's peak-hour buckets at its peak rates. A bucket sums calls and says nothing of any one prompt's length, so buckets are priced at the base rates, and a range can come out below its sessions where prompts ran past a tier. Only the selected kinds are priced, and models without a list price are named, not counted. The agent cards rank agents, models and sessions by their tokens of the selected kinds, the international list price breaking ties, and price every kind whatever the selection.
