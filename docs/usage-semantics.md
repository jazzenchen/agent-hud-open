...
- The Tokens page prices the charted range and each period from 15-minute buckets, each model on its client's platform and DeepSeek's peak-hour buckets at its peak rates. A bucket sums calls and says nothing of any one prompt's length, so buckets are priced at the base rates, and a range can come out below its sessions where prompts ran past a tier. Only the selected kinds are priced, and models without a list price are named, not counted.
- The periods (`UsagePeriods`) are today from local midnight and the last seven and thirty days, read from the ledger, which keeps 31 days of buckets; the charts' own report covers one week.
