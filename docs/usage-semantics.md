...
- The Tokens page prices the charted range and each period from 15-minute buckets. A bucket sums calls and says nothing of any one prompt's length, so buckets are priced at the base rates, and a range can come out below its sessions where OpenAI prompts ran past 272K tokens. Only the selected kinds are priced, and models without a list price are named, not counted.
